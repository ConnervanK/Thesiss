...
(\cref{sec:meth-resolution,ch:hyp1,ch:hyp2}).
\Cref{sec:th-fourier-shift}--\ref{sec:th-local} then derive the 2D phase-plane
shift-estimation method that is validated in \cref{sec:hyp1-phaseplane} and applied
to a realistic scenario in \cref{ch:hyp2}: starting from the Fourier shift
...
(\cref{sec:meth-resolution}) is, in principle, exactly recoverable from the phase of
the spectrum.
...
\end{equation}
This orthogonality is exploited directly in \cref{ch:hyp2}, where the
...
acts on the whole 2D spectrum at once. \Cref{sec:hyp1-phaseplane} also examines a
...
The phase-plane fit treats the migrated image as a whole. \Cref{sec:hyp1-phaseplane}
...
\subsection{Three diagnostic views used in Chapter~\ref{sec:hyp1-phaseplane}}

Three derived plots make \cref{eq:local-phase-line} directly visible in the
data, and are used repeatedly in the figures of \cref{sec:hyp1-phaseplane}:
...
in \cref{sec:hyp1-phaseplane} to localise \emph{where} (in $\tau$) a phase
