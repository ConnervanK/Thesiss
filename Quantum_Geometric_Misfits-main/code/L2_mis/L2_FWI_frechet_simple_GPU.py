...
# Output folders
output_folder = "figures/fwi_L2_frechet_simple_GPU"
...
def main():
    ...
    for freq_hz, n_iter_stage in schedule:
        ...
        for iter_sub in range(n_iter_stage):
            ...
            # --- Current State ---
            kappa_curr = (v_current.flatten() ** 2) * rho_bg
            ...
            print(f"Freq {freq_hz}Hz | Iter {total_iter_count:02d} | Loss: {total_sq:.4e} | Geom: {comp_A:.4e} | Param: {comp_B:.4e}")
            ...
            # Base loss on GPU
            # (Note: we computed total_sq above on CPU using give_misfits, but for consistent differences we should use GPU loss or recompute)
            # Let's compute current_loss on GPU to be safe against slight numerical differences
            current_loss_gpu = gpu_simulation_loss(
                    v_current, rho_bg, rho_list, A_geom_gpu, dt, nt, 
