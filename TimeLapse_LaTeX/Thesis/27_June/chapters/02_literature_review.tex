...
hypothesis of \cref{ch:introduction} and the four specific hypotheses tested
in \cref{ch:hyp1,ch:hyp2,ch:hyp3} --- add literature on (1) time-lapse/4D
...
\section{EM Theory and GPR Physics}
...
\section{Resolution Problems (Lateral and Vertical) and the Fresnel Zone}
\label{sec:lit-resolution}

Two distinct resolution criteria limit what a GPR survey can image, and both
are tested empirically in this thesis.

\paragraph{Lateral resolution.} The lateral resolving power of an unmigrated
survey is governed by the \emph{Fresnel zone} --- the region of the
...
approximately $a_F \approx \sqrt{\lambda d/2}$ \citep{annan2009}; two
reflectors closer together than this radius cannot be distinguished without
migration, since their Fresnel zones overlap. Migration (\cref{ch:theory})
...
quantified empirically in \cref{sec:meth-resolution} and tested as a
time-lapse detection problem in \cref{ch:hyp1}.

\paragraph{Vertical resolution.} Vertical (depth) resolution is instead
governed by the \emph{temporal} bandwidth of the source pulse: two
...
throughout this thesis \citep{daniels2004}. Unlike lateral resolution, this
...
\cref{ch:hyp1} are treated as genuinely distinct experiments rather than as
trivial rotations of one another (\cref{sec:th-duality}).

\section{GPR Processing}

\paragraph{Migration algorithms.} Three families of migration algorithm are
used throughout this thesis, and each has an established lineage in the
geophysical literature. \citet{claerbout1985} introduces the
exploding-reflector model that underlies the zero-offset migration
convention adopted in \cref{ch:theory}. \citet{schneider1978} formulates
...
Kirchhoff operator. \citet{gazdag1978} develops the
frequency--wavenumber phase-shift migration algorithm used as an
independent wave-equation-based cross-check; \citet{stolt1978} is the
...
\cref{ch:hyp1,ch:hyp2,ch:hyp3} independent of any single migration
assumption.

\paragraph{Simulation tooling.} All synthetic data in this thesis are
generated with gprMax \citep{gprmax}, an open-source finite-difference
...
lets the back-propagation (time-reversal) migration of \cref{ch:theory} be
