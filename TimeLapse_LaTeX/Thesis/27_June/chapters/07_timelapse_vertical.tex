...
\paragraph{Hypothesis.} The same amplitude-differencing detectability floor
found for lateral displacement (\cref{ch:tl-horizontal}) applies to
\emph{vertical} displacement as well --- but, because a migrated image
separates lateral spatial frequency like a prism while leaving vertical
frequency unseparated (\cref{sec:th-duality}), the vertical case is not
expected to behave identically to the lateral one.
