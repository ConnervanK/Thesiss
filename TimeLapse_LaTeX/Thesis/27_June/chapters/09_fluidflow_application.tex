\chapter{Application to a Realistic Fluid-Flow Scenario}
\label{ch:fluidflow}

The preceding chapters used an idealised target: a point-like PEC cylinder,
chosen because its response is easy to interpret and ground-truth. This
chapter applies the same migration and phase-plane machinery to a more
...
\paragraph{Hypothesis.} The same migration and phase-plane machinery,
applied to a spatially \emph{distributed} fluid front rather than a rigid
point scatterer, still decouples a purely geometric displacement from a
purely dielectric material change, extending the geometry/material-change
decoupling of \cref{sec:th-material-change} beyond an idealised point
target.
...
tests something qualitatively different from \cref{ch:phase-plane}: rather
than recovering a single $(\Dz,\Dx)$ displacement, the goal is to confirm
that the fit correctly attributes the front's advance to the
