...
The resolution of a single GPR image is bounded by a wavelength-scale floor: any displacement of a scatterer, or advance of fluid within a fracture, smaller than the dominant wavelength cannot be resolved as a distinct spatial feature in one survey — the target simply stays embedded within a single Fresnel zone or range cell. This is the central obstacle this thesis addresses, and the reason it turns to _time-lapse_ acquisition: rather than resolving sub-wavelength motion directly in one image, repeated surveys exploit the small but detectable change the motion produces in the reflected wavefield — a change amplified by the strong permittivity contrast of water (Milsom, 2011).

The floor itself has two independent parts. Vertical (range) resolution is bounded by the pulse width and material velocity (Δr ≥ Wv/4), while lateral resolution is governed by the Fresnel zone and so degrades with target depth, following relations such as Δl ≥ √(vrW/2) (Annan, 2009). The exact closed form is not settled: Rial (2007) found a Fresnel-zone formulation best matched the observed horizontal resolution of bow-tie antennas, and Pérez-Gracia (2008) distinguished the absolute resolution of a system from the broader antenna footprint needed for clearly separated, interference-free images — both of which remain coarse relative to the scale of an individual scatterer.

== Amplitude-Based Migration: The Traditional Answer to Resolution <sec:lit-amplitude>

Migration is the classical response to the resolution problem posed above: rather than reading reflection amplitudes directly off an unfocused B-scan, it repositions recorded energy into its true subsurface location, collapsing the diffraction hyperbola generated by a point scatterer back toward a focused point wherever the source and receiver wavefields coincide in space and time (Jones, 2014). This thesis implements three such algorithms — Kirchhoff delay-and-sum migration, Gazdag frequency-wavenumber phase-shift migration, and back-propagation (time-reversal) migration — as PyLops-based operators, and each is derived in full in @ch:theory. What matters for the argument here is only what all three have in common: every one of them builds its image from the amplitude and kinematic timing of the recorded wavefield alone, whether by weighted backprojection onto isochron surfaces (Kirchhoff), by downward-continuation with a phase-shift operator (Gazdag), or by time-reversed re-propagation (back-propagation) (Li, 2020; Özdemir, 2014). Because none of these imaging conditions exploits anything beyond amplitude and traveltime, the resolution they can deliver is bounded by the same Fresnel-zone / Rayleigh floor identified in @sec:lit-resolution: migration relocates the footprint, but does not by itself shrink it (Jones, 2014; Li, 2020).
