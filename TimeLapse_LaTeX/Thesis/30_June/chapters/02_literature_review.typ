...
(@sec:lit-fundamentals); that a wavelength-scale floor nonetheless bounds the resolution of a single GPR image, so sub-wavelength change is
...
Ground-penetrating radar is a _wave-field_ method: unlike the diffusive electrical techniques, it propagates a true wave whose velocity (c = (εμ)⁻¹/²) and reflection strength are set by the subsurface electrical properties @slob2024 @annan2009. The property that matters most here is permittivity, and what most controls permittivity is water: liquid water has a relative permittivity of about 80 against 3–8 for most dry minerals, so water content dominates both the wave velocity and the reflection coefficients generated at interfaces @milsom2011gpr. This is what makes GPR a sensitive probe of fluid-related change — fluid entering a fracture, say — and it is the physical basis for reading a time-lapse change in the reflected wavefield as evidence of a subsurface change in water content. Underlying this, GPR obeys Maxwell's equations through the constitutive parameters σ, ε, and μ @annan2009, and is effective only in low-loss media where displacement currents dominate over ohmic dissipation, across the HF–UHF bands (roughly 10 MHz–4 GHz).

== Resolution problems and their bearing on sub-wavelength movement <sec:lit-resolution>

A wavelength-scale floor bounds the spatial resolution of a single GPR image: static features smaller than the dominant wavelength—such as the separation between two scatterers or the width of a fluid-filled fracture—cannot be resolved as distinct entities. They remain embedded within a single Fresnel zone or range cell. This static resolution limit is the central obstacle this thesis addresses, motivating the shift to _time-lapse acquisition_. Rather than attempting to resolve sub-wavelength structures in a single baseline survey, repeated monitoring surveys exploit the small but detectable wavefield changes produced by moving scatterers or fluid advance—changes heavily amplified by the strong permittivity contrast of water @milsom2011gpr.
This resolution floor consists of two independent components. The pulse width bounds vertical (range) resolution ($Delta r$) at half amplitude ($W$) and the material velocity ($v$), such that $Delta r >= (W v) / 4$. The Fresnel zone governs lateral resolution ($Delta l$) and degrades with target depth ($r$), following relations such as $Delta l >= sqrt((v r W) / 2)$ @annan2009. By relating $W$ to the bandwidth ($B$) and centre frequency ($f_c$) as $W approx 1/B approx 1/f_c$, and defining the centre wavelength as $lambda_c = v/f_c$, the lateral limit can be expressed as $Delta l = sqrt((r lambda_c) / 2)$. Consequently, two point reflectors buried at the same depth cannot be resolved as separate anomalies if their lateral separation is smaller than the diameter of this antenna footprint @rial2007 @perezgracia2008.
The exact closed form is not settled: #cite(<rial2007>, form: "prose") found a Fresnel-zone formulation best matched the observed horizontal resolution of bow-tie antennas, and #cite(<perezgracia2008>, form: "prose") distinguished the absolute resolution of a system from the broader antenna footprint needed for clearly separated, interference-free images — both of which remain coarse relative to the scale of an individual scatterer.

== Amplitude-Based Migration: The Traditional Answer to Resolution <sec:lit-amplitude>

Migration is the classical response to the resolution problem posed above: rather than reading reflection amplitudes directly off an unfocused B-scan, it repositions recorded energy into its true subsurface location, collapsing the diffraction hyperbola generated by a point scatterer back toward a focused point wherever the source and receiver wavefields coincide in space and time @jones2014. This thesis implements three such algorithms — Kirchhoff delay-and-sum migration, Gazdag frequency-wavenumber phase-shift migration, and back-propagation (time-reversal) migration — as PyLops-based operators, and each is derived in full in @ch:theory. What matters for the argument here is what all three have in common: although they rely on phase shifts and traveltimes internally to focus the wavefield—whether by weighted backprojection onto isochron surfaces (Kirchhoff), downward-continuation with a phase-shift operator (Gazdag), or time-reversed re-propagation (back-propagation) @li2020 @ozdemir2014—they produce a single structural snapshot. Because a standard static image relies strictly on the focused amplitude to define boundaries, the resolution it can deliver is bounded by the same Fresnel-zone/Rayleigh floor identified in @sec:lit-resolution: migration relocates the footprint but does not, by itself, shrink it @jones2014 @li2020.

A large body of work tries to push this floor lower. Some methods stay within the amplitude imaging condition: deconvolution compresses the source wavelet to separate interfering arrivals @schmelzbach2015 @moghaddam2019; reverse-time migration and its least-squares refinement replace ray-based assumptions with a full wave-equation imaging condition @jones2014 @liu2024; and full-waveform inversion inverts the entire recorded waveform for quantitative permittivity and conductivity, in principle exceeding the Fresnel-zone limit @meles2011 @wang2025. Others abandon the amplitude imaging condition altogether for subspace decomposition — MUSIC and related sparsity methods @li2025 @gao2022 — the only class here that demonstrably beats the classical limit rather than merely approaching it. But that gain is bought with structure a routine survey does not supply for free: a multistatic data matrix, an engineered focusing geometry, or an assumed wavelet and background velocity model that a single zero-offset time-lapse survey cannot guarantee.
